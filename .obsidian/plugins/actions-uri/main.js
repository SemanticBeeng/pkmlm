...
async function appendNoteBelowHeadline(filepath, belowHeadline, textToAppend, shouldEnsureNewline = false) {
  const escapedHeadline = belowHeadline.replace(/[|\\{}()[\]^$+*?.]/g, "\\$&").replace(/-/g, "\\x2d").replace(/\s+/g, "\\s+");
  const searchTerm = new RegExp(`([\\n^]${escapedHeadline})(\\s*\\n.*?)(\\n#+ |$)`, "s");
  const escapedReplacement = (shouldEnsureNewline ? endStringWithNewline(textToAppend) : textToAppend).replace(new RegExp(/\$/, "g"), "\\$");
...
async function prependNoteBelowHeadline(filepath, belowHeadline, textToPrepend, shouldEnsureNewline = false) {
  const escapedHeadline = belowHeadline.replace(/[|\\{}()[\]^$+*?.]/g, "\\$&").replace(/-/g, "\\x2d").replace(/\s+/g, "\\s+");
  const searchTerm = new RegExp(`([\\n^]${escapedHeadline})(\\s*\\n)(.*?(\\n#+ |$))`, "s");
  const escapedReplacement = (shouldEnsureNewline ? endStringWithNewline(textToPrepend) : textToPrepend).replace(new RegExp(/\$/, "g"), "\\$");
  const replacement = `$1$2${escapedReplacement}$3`;
  return await searchAndReplaceInNote(filepath, searchTerm, replacement);
}
...
async function handleAppend(incomingParams) {
  ...
  async function appendAsRequested(filepath) {
    if (belowHeadline) {
      return await appendNoteBelowHeadline(filepath, belowHeadline, content, ensureNewline);
    }
...
var PLUGIN_INFO = {
  "pluginVersion": "1.2.2",
  "pluginReleasedAt": "2023-07-13T16:07:36+0200"
};
...
async function handleAppend2(incomingParams) {
  ...
  async function appendAsRequested() {
    if (belowHeadline) {
      return await appendNoteBelowHeadline(file, belowHeadline, content, ensureNewline);
    }
